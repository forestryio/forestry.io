...
_Why Vuepress?_ Vuepress' default support for .md is similar to Hugo and Jekyll behavior which makes it a great candidate for our content editor. We already made a lot of changes (to the sidebar, imports etc.), so jumping to an SSG that doesn't have that default felt like we were skipping a step.
