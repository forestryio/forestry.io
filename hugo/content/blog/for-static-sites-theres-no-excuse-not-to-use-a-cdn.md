...
Communicating with the more distant server caused a *600% increase* in round-trip latency. 250 milliseconds may sound like a short amount of time, but 1/4 of a second is long enough for a user to notice. Not only that, this time penalty applies to *every request* the user makes to that server: CSS, JavaScript, images, etc. If there were an easy way to eliminate this latency, wouldn’t you want to do it?
...
If the previous section has you worried that putting your site on a CDN is complicated, don’t worry. [Netlify](https://www.netlify.com/) makes it easy! Their service takes care of deploying and hosting your static site, and setup takes just a few steps. The video below shows you how easy it is to get started with Netlify:
