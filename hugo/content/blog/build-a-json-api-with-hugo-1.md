...
---
In the first part of this article we built a fully functional GET API. Hugo makes it super easy to build simple APIs with its built-in output formats. Today we’re going to add functionalities to our API and put it more in tune with modern APIs. 

At the moment our API can only look up specific items but can’t look at them in relation to each other. Say you want to know what players are in a specific team? With our current API you won’t get a simple answer to that question. To solve this we’ll use Custom Output Formats to create another JSON available to read and use such “relationships”.
...
# 1 Connecting Models, and using their relationship in our template.
...
Second, the most obvious way to look up a team would be by their name, i.e. `sly-turtles` or `flying-toasters`. However, if the team names look like this `🐯❤️🐈`  or `cool-tiger-sparkly-cat-hot-fish-quick-whale-wet-snake-boring-baracudas` - looking up `teams`  could quickly become cumbersome and error prone. We like to make it a little simpler.
...
Now that our players and teams are linked, we want to list every players of one a given team. We already have an endpoint URL for one the team, using Hugo’s JSON Built-In Output Format: `school.api/teams/sly-turtles/index.json`.
...
So in `layouts/teams/` we’ll create `single.players.json` and add this code to it. I explain the following lines in detail below:
        ...
        {{ define "response" }}
        {{- $players := where (where .Site.RegularPages "Params.team" .uniqueID) "Type" "players" -}}
            {
                "team" : "{{ .Title }}",
                "count" : "{{ len $players }}"
                ,"players" : [
...
2 Create Endpoint URLs to Access Players filtered by a Taxonomy
...
We can use our previous template code with only one difference. Since we’re in a list context, we can use `.Data.Pages` instead of `.Site.RegularPages`. The only drawback is that it will contain all pages that are put in relation to our taxonomy (sport). So just like in our `single.players.json` we need to use a `where` statement to filter our base data to pages of Type `players`. 
...
2.2 Available to any taxonomy
Now if we were to create a new taxonomy, say `practice` to identify the weekdays when each player practices, this taxonomy will instantly inherit the `players` Custom Output Format already assigned to taxonomies.
Let’s try it.
In our `config.toml`:
...
Go to http://localhost:1313/practices/thursday/players.json

3 Beautification of URLs to better match modern APIS
...
2. Our API would be more intuitive if looking up players by team or sport used the same route as looking up all players, since the type of data being returned is fundamentally the same.
...
We’ll start off easy with the first step and will venture into some of the more complex issues of URL rewriting further down.
...
Fortunately, Netlify gives us the serverless power we are looking for and also allows us to rewrite URLs. To handle these rewrites, Netlify looks for a file called `_redirects` in the root of our website. This file uses a relatively simple syntax to define URL rewrites. For example this is one of their rewriting rule syntax:  `/players team=:id`.
You can easily guess that `/players` is the url, `team` is the query parameter and `:id` is the passed value.

For those of you not familiar with Netlify, the Hugo website has a great start up guide for you to look out. 
...
2. The second part is the URL the matched string that should be called. Here `:splat` is where the matched string will go, just before our `index.json`
3. The third part, is the code. `200` means it is not a redirect, but a rewriting.

From now on Netlify will automatically point any URL’s to their `index.json`. Meaning `school.api/players/index.json` will be called via `school.api/players/`, `api.com/players/jacky-f-robinson/index.json` will be called via `school.api/players/jacky-robinson/` etc…
...
Note that we added the new « team » rule at the top. Our redirect file needs to be organized from the bottom up and from general to specific. It is important that the general « splat » rule remains at the bottom so it does not override the more specific « team » rule.
3.3 Listing our players by sports or trainings or any taxonomy?
The last part of this chapter is concerned with fixing the URLs for our `players` lists that are filtered by our taxonomies `sports` and `trainings`.
...
But this would mean modifying by hand the `_redirects` file each time a new taxonomy is added to the project. We cannot expect the content manager or even the dev team to modify `_redirects` every time we need an extra layer of information on a player.
...
For now, we’ve used a Built-In Output Format for our `index.json,` and a Custom Output Format for our `players.json`. However, they both use a common Built-In Media Type: JSON.
...
The first line adds a Media Type called `text/netlify`, you can call it whatever you like, but I like to match the media type convention, hence the `/` in the middle.
`suffix` is the extension, here we don’t have any extension, so it’s blank.
`delimiter` is whatever joins the extension to the file name. There is no extension, hence we don’t need a delimiter, it’ll stay blank.
...
You can call it whatever you’d like but for our purposes `redirect` makes the most sense.
...
Then, all we have to do in between those lines is to iterate over our site taxonomies to create the redirect rules.
...
and not `/players/?sport``s``=football`
Those `-` attached to the opening of our `range` and `end` curlies is to make sure Hugo does not add too many line breaks.
...
Of course as previously mentioned, there are many other server technologies out there and all come with their particular URL rewriting logic. This Netlify introduction detailed above was only one of them. But after experimenting with it, it does not take much to add another rewriting file like `.htaccess` for apache or `Web.config` for DotNet.

All you’d have to do is create the dedicated Custom Output Format and Media Type to have Hugo generate the redirect file your server environment needs.

API living on top of a conventional HTML website.

As seen in Part 1 it does not take any more work to make this API lives alongside a conventional HTML static site. Just one thing to remember when dealing with URL Rewriting.
...
To make sure our JSON files cohabit nicely with our HTML files we have to distinct our page URLs from our API URLs. For this, we’ll prepend every API URLs with something like `/api/`.
Every redirect rule including our bottomline universal one should reflect this. 
Provided your website is more or less conventional like our school’s, this is what you’d have in your `layouts/_default/index.redirect`:
        ...
        # Every api/payers/?team={team} rewrites to /teams/{team}/players.json
